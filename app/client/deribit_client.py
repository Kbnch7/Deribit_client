...
class DeribitClient:
    ...
    async def get_index_price(self, index_name: str) -> float | None:
            ...
            try:
                async with session.get(url, params=params) as response:
                    if response.status == 200:
                        ...
                        print(
                            f"Error fetching price for {index_name}: {response.status}"
                        )
                        return None
...
def sync_get_index_price(index_name: str) -> float | None:
